...
async def list_support_tickets_async(
    db,
    *,
    status: str | None = None,
    q: str = "",
    ...
    stmt = select(SupportTicket).order_by(SupportTicket.updated_at.desc(), SupportTicket.id.desc())
    if status and status in TICKET_STATUSES:
...
async def ticket_detail_async(db, ticket_id: int) -> dict | None:
    ...
    assignee = await db.get(User, ticket.assigned_to_user_id) if ticket.assigned_to_user_id else None
    return {"ticket": ticket, "messages": messages, "user": user, "assignee": assignee}


async def reply_support_ticket_async(
    db,
    ticket_id: int,
    *,
    author_user_id: int,
...
async def set_ticket_status_async(db, ticket_id: int, status: str) -> tuple[SupportTicket | None, str | None]:
    status = (status or "").strip()
    if status not in TICKET_STATUSES:
        return None, "Некорректный статус"
    ticket = await db.get(SupportTicket, ticket_id)
