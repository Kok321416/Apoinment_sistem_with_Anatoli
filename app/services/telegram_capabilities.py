...
from app.services.integration_telegram import (
    find_integration_by_chat_id,
    find_integration_by_chat_id_async,
...
async def resolve_capabilities_async(db, *, telegram_id: int | str | None, telegram_chat_id: str | None) -> dict:
    ...
    is_client = False
    if tid is not None:
        booking_row = await db.execute(select(Booking.id).where(Booking.telegram_id == tid).limit(1))
        if booking_row.first():
            is_client = True
        else:
            sa = (
                await db.execute(
    ...
    is_specialist = False
    if chat_key and await find_integration_by_chat_id_async(db, chat_key):
        is_specialist = True
    elif tid is not None:
        sa = (
            await db.execute(
        ...
        if sa:
            consultant_row = await db.execute(
                select(Consultant.id).where(Consultant.user_id == sa.user_id).limit(1)
            )
            if consultant_row.first():
                is_specialist = True
    ...
    stored = None
    if chat_key:
        pref = await db.get(TelegramUiPreference, chat_key)
        if pref and pref.mode in VALID_MODES:
            stored = pref.mode
...
async def get_ui_mode_async(db, chat_id: str) -> str | None:
    key = normalize_telegram_chat_id(chat_id)
    if not key:
        return None
    pref = await db.get(TelegramUiPreference, key)
    if pref and pref.mode in VALID_MODES:
        return pref.mode
    return None


async def set_ui_mode_async(db, chat_id: str, mode: str) -> tuple[bool, str]:
    key = normalize_telegram_chat_id(chat_id)
    if not key:
        return False, "chat_id required"
    mode = (mode or "").strip().lower()
