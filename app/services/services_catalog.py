...
async def booking_counts_async(db, service_ids: list[int]) -> dict[int, int]:
...
async def analytics_panel_async(db, services: list[Service], consultant_id: int) -> dict:
    from sqlalchemy import select
    ...
    if service_ids:
        total_bookings = (
            await db.execute(
                select(func.count(Booking.id)).where(Booking.service_id.in_(service_ids))
            )
        ).scalar_one() or 0
        popular = (
            await db.execute(
...
async def service_statistics_async(db, service: Service) -> dict:
    from sqlalchemy import select
...
async def build_catalog_payload_async(db, consultant_id: int, *, use_cache: bool = True) -> dict:
    from sqlalchemy import select

    from app.services.response_cache import TTL_SEC, catalog_key
    from app.services.ttl_cache import CACHE

    cache_key = catalog_key(consultant_id) if use_cache else None
    if cache_key:
    ...
    services = list(
        (
            await db.execute(
                select(Service)
                .where(Service.consultant_id == consultant_id)
                .order_by(Service.sort_order, Service.name)
            )
        )
        .scalars()
    ...
    calendars = list(
        (
            await db.execute(
                select(Calendar)
                .where(Calendar.consultant_id == consultant_id)
                .order_by(Calendar.name)
            )
        )
        .scalars()
...
async def next_sort_order_async(db, consultant_id: int) -> int:
    from sqlalchemy import select

    max_order = (
        await db.execute(
            select(func.max(Service.sort_order)).where(Service.consultant_id == consultant_id)
        )
    ).scalar_one()
    return (max_order or 0) + 1
